...
import { FloatingHydrationMascot } from "@/components/floating-hydration-mascot";
...
export function WaterHome() {
  ...
  function recordDrink(nextAmount: number) {
    ...
    const feedback = feedbackFor(
      ...
      nextAmount,
      now,
      state.reminders.startTime,
      state.reminders.endTime,
    );
    ...
    mascotTimerRef.current = window.setTimeout(
      () => setActionMode(null),
      feedback.mode === "celebrate" ? 1300 : 1050,
    );
  ...
  return (
    <MobileShell>
      ...
      <div className="px-5">
        ...
        <section className="mt-7" aria-labelledby="quick-add">
          ...
          <div className="mt-3 grid grid-cols-3 gap-2" aria-labelledby="quick-add">
            <span id="quick-add" className="sr-only">เพิ่มปริมาณน้ำแบบด่วน</span>
          ...
          <Link href="/reminders" className="block rounded-2xl outline-none focus-visible:ring-2 focus-visible:ring-ring">
            <Card className="border-0 bg-secondary/70 shadow-none transition hover:bg-secondary">
              <CardContent className="flex items-center gap-4 py-4">
                <HydrationMascot
                  mode={state.reminders.enabled ? "reminder" : ambientMode}
                  hydrationRatio={ratio}
                  size={48}
                  subtle
                />
                <div className="min-w-0 flex-1">
                  {nextReminder ? (
                    <>
                      ...
                      <p className="mt-1 text-sm font-medium text-foreground">{formatRelativeMinutes(nextReminder, now)}</p>
        ...
        <section className="mt-8" aria-labelledby="today-history">
          <div className="flex items-center justify-between">
            <h2 id="today-history" className="text-base font-semibold tracking-[-0.02em]">วันนี้</h2>
      ...
      <FloatingHydrationMascot
        animationKey={`${mascotRun}-${mascotMode}`}
        mode={mascotMode}
        hydrationRatio={ratio}
      />
      ...
      <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
        <SheetContent
          side="bottom"
          className="mx-auto w-full max-w-[430px] pb-[calc(1rem+env(safe-area-inset-bottom))] sm:left-1/2 sm:-translate-x-1/2"
        >
          <SheetHeader>
